    ...
    def subword_tokenize(self, tokens, labels, head, arc_label):  # for tag split
        ...
        for ix, token in enumerate(tokens):
            ...
            for jx, sub_token in enumerate(sub_tokens):
                ...
                if jx == 0:
                    split_labels.append(labels[ix] + '_B') 
                else:
                    split_labels.append(labels[ix] + '_I')

                # dependency relation and head index
                if jx == 0:
                    first_token_map[ix] = len(split_tokens) - 1  
    ...
    while ix < len(document): 
        ...
        if True: 
            if current_chunk:
                ...
                rel = [-1] + rel_a + [-1]  
                assert len(tokens) == len(heads) == len(tags), print('长度不等！')

                for i in range(len(heads)):
                    head_tokens = heads[i]
                    if head_tokens == 0:
                        rel[i] = -1  # if the head token does not exist the dependency label set to : -1
                    assert head_tokens >= 0 and head_tokens < max_seq_length

                assert len(heads) == len(tokens) == len(tags), print('长度不等！', len(heads), len(tokens), '!!!!')
...
def parse_tree(doc):
    # get the head token index for each word
    head_index = []
    for ix, t in enumerate(doc):
        ...
        for jx, h in enumerate(doc):
            if t.head.text == h.text and t in [c for c in h.children]:
                ...
                break
        if prev == len(head_index):  # for root token
